...
// 使用短 UUID 格式
const BREATHING_SERVICE_UUID = 'ff01'
const BREATHING_CHAR_UUID = 'ff02'
const BREATHING_CMD_UUID = 'ff03'
...
class BluetoothService {
  ...
  async connectBreathingBand() {
    try {
      ...
      const bluetoothDevice = await navigator.bluetooth.requestDevice({
        ...
        optionalServices: [BREATHING_SERVICE_UUID]
      })
      ...
      console.log('获取到主服务:', service.uuid)
      
      this.breathingCharacteristic = await service.getCharacteristic(BREATHING_CHAR_UUID)
      console.log('获取到特征值:', this.breathingCharacteristic.uuid)
      ...
      // 发送启动命令
      const cmdChar = await service.getCharacteristic(BREATHING_CMD_UUID)
      console.log('获取到命令特征值:', cmdChar.uuid)
  ...
  handleBreathingChange(event) {
    const value = event.target.value
    const breathingValue = this.parseBreathingData(value)
    console.log('收到呼吸数据:', breathingValue)
