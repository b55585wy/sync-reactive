...
class BluetoothService {
  ...
  async connectBreathingBand() {
    try {
      ...
      const bluetoothDevice = await navigator.bluetooth.requestDevice({
        filters: [
          { services: [BREATHING_SERVICE_UUID] }
        ],
        optionalServices: [BREATHING_CMD_UUID]
      })

      console.log('找到呼吸带设备:', bluetoothDevice.name)
      ...
      // 发送启动命令
      const cmdService = await server.getPrimaryService(BREATHING_CMD_UUID)
      const cmdChar = await cmdService.getCharacteristic(BREATHING_CMD_UUID)
  ...
  async disconnectBreathingBand() {
    if (this.breathingCharacteristic) {
      try {
        // 发送停止命令
        const cmdService = await this.device.value.gatt.getPrimaryService(BREATHING_CMD_UUID)
        const cmdChar = await cmdService.getCharacteristic(BREATHING_CMD_UUID)
