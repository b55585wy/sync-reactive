...
// 添加心跳动画相关的样式
const getHeartbeatStyle = computed(() => {
  return {
    animation: isBeating.value ? 'heartbeat 0.2s ease-in-out' : 'none'
  };
});
...
watch(() => deviceStore.currentHeartRate, (newRate) => {
  ...
  if (newRate > 0) {
    const interval = 60000 / newRate; // 根据心率计算间隔时间
    heartbeatTimer = setInterval(() => {
      isBeating.value = true;
      setTimeout(() => {
        isBeating.value = false;
