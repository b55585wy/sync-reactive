        ...
        <div class="zone-distribution">
          ...
          <div class="zone-bars">
            ...
            >
              ...
              <el-tooltip :content="zone.description" placement="top">
...
const currentHeartRateZone = computed(() => {
  const hr = deviceStore.currentHeartRate;
  return heartRateZones.value.find(zone => 
    hr >= zone.range[0] && hr <= zone.range[1]
  ) || null;
});

const zoneDistribution = computed(() => {
  const total = heartRateHistory.value.length;
  if (total === 0) return [];
  
  return heartRateZones.value.map(zone => ({
    ...zone,
    percentage: getZonePercentage(zone.range)
  }));
});

// 更新 isInZone 方法以使用新的区间定义
const isInZone = (range: [number, number]) => {
  const hr = deviceStore.currentHeartRate;
  return hr >= range[0] && hr <= range[1];
};
...
// 添加获取训练建议的方法
const getTrainingAdvice = computed(() => {
  const currentZone = currentHeartRateZone.value;
  if (!currentZone) return null;

  const targetMin = settingsStore.targetHeartRateMin;
  const targetMax = settingsStore.targetHeartRateMax;
  const hr = deviceStore.currentHeartRate;
  ...
  if (hr > targetMax) {
    return {
      type: 'warning',
      title: '心率偏高',
      content: `当前处于${currentZone.name}区间，建议调整呼吸节奏降低心率`
    };
  }
  ...
  if (hr < targetMin) {
    return {
      type: 'info',
      title: '心率偏低',
      content: `当前处于${currentZone.name}区间，可以适当提升运动强度`
    };
  }
  ...
  return {
    type: 'success',
    title: '心率达标',
    content: `当前处于${currentZone.name}区间，保持当前状态`
  };
});
</script>
