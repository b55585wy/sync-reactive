...
export const useBreathingStore = defineStore('breathing', {
  ...
  getters: {
    currentPhaseTime() {
      const { phase, mode } = this.currentSession
      if (mode === 'guided') {
        return this.settings.guidedMode[`${phase}Time`] || 0
      }
      return this.settings.customMode[`${phase}Time`] || 0
    },
    
    sessionProgress() {
      if (!this.currentSession.totalTime) return 0
      return (this.currentSession.elapsedTime / this.currentSession.totalTime) * 100
    }
  }
}) 